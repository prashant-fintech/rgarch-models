...
price_column <- 2
output_prefix <- "reliance_vol_estimates_GARCH"
...
# Function to fit GARCH model and generate results
fit_garch_model <- function(returns, dist_model, output_file) {
  spec <- ugarchspec(
...
garch_fit_norm <- fit_garch_model(returns, "norm", paste0(output_prefix, ".csv"))

# GARCH model with Student t distribution
cat("\n=== GARCH with Student t distribution ===\n")
garch_fit_std <- fit_garch_model(returns, "std", paste0(output_prefix, "_t.csv"))
cat("\nStudent t quantiles:\n")
calculate_quantiles(garch_fit_std, "std")

# GARCH model with GED distribution
cat("\n=== GARCH with GED distribution ===\n")
garch_fit_ged <- fit_garch_model(returns, "ged", paste0(output_prefix, "_ged.csv"))
cat("\nGED quantiles:\n")
calculate_quantiles(garch_fit_ged, "ged")

# GARCH model with Skewed Normal distribution
cat("\n=== GARCH with Skewed Normal distribution ===\n")
garch_fit_snorm <- fit_garch_model(returns, "snorm", paste0(output_prefix, "_snorm.csv"))
cat("\nSkewed Normal quantiles:\n")
calculate_quantiles(garch_fit_snorm, "snorm")

# GARCH model with Skewed Student t distribution
cat("\n=== GARCH with Skewed Student t distribution ===\n")
garch_fit_sstd <- fit_garch_model(returns, "sstd", paste0(output_prefix, "_st.csv"))
cat("\nSkewed Student t quantiles:\n")
calculate_quantiles(garch_fit_sstd, "sstd")

# GARCH model with Skewed GED distribution
cat("\n=== GARCH with Skewed GED distribution ===\n")
garch_fit_sged <- fit_garch_model(returns, "sged", paste0(output_prefix, "_sged.csv"))
